...
rule bwa_sample:
    ...
    shell:
        """
        script/launch_bwa.sh -r {input.contigs} -1 {input.fastq} \
        -o {output.prefix}
        """
...
rule mutect:
    ...
    shell:
        """
        script/launch_mutect.sh -a {input.contigs} -r {input.bamr} \
        -i {input.bams} -o {output}
        """
...
rule merge_results:
    ...
    shell:
        """
        script/pipeline_output.py {params} -r {input.contigs} -i {input.stats} \
        -o {output}
        """
